...
class Food:
    def __init__(self):
        ...
        canvas.create_text(x + SPACE_SIZE // 2, y + SPACE_SIZE // 2, text=FOOD_EMOJI, font=('Arial', 20), fill="white",
                           tags="food_text")
...
def check_collisions(snake):
    ...
    return x < 0 or x >= WIDTH or y < 0 or y >= HEIGHT or any(
        x == body_part[0] and y == body_part[1] for body_part in snake.coordinates[1:])
...
def game_over():
    ...
    global play_button
    play_button = Button(window, text="READY?", font=('consolas', 20), command=retry_game, border=5, width=8,
                         relief='flat')
